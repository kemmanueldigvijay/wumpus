...
public class GUI implements ActionListener
{
    ...
    private void createWindow()
    {
        ...
        buttons.add(bs);
        JButton bb = new JButton("Run Training Agent");
        bb.setActionCommand("TRAINING");
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        if (e.getActionCommand().equals("TRAINING")) {
            if (agent == null) {
                ...
                initializeMap(CurrentTime);
                agent = new MyAgent(w);
                System.out.println("Training Agent");
                for (int i = 0; i < QalgoConfigure.ITERATIONS; i++) {
                    agent.trainAgent();
                    updateGame();
                    if (w.gameOver()) {
                        initializeMap(CurrentTime);
    ...
    /**
     * Initialize and re-initial map
     */
    private void initializeMap(int CurrentTime) {
        String s = (String) mapList.getSelectedItem();
        if (s.equalsIgnoreCase("Random")) {
            w = MapGenerator.getRandomMap(CurrentTime).generateWorld();
        } else {
            int i = Integer.parseInt(s);
            i--;
            w = maps.get(i).generateWorld();
